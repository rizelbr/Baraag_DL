...
def user_login(client, code):
    ...
    code = OAuth token (str). Provided by user via oauth_exec()
              REQUIRED
    ...
    """
    client.log_in(scopes = ["read"],
                    code = code,
                    to_file = "user_credentials")

def login_loop(client, code):
    """
    ...
    not provide an authentication code.
    
    """
    try:
        if code:
            user_login(client, code)
            print()
        ...
        if "'invalid_client'" in str(exc):
            ...
            print()
            print(Fore.RED+"Unable to login with the code provided!"+Fore.RESET)
            print()
            print(Fore.YELLOW+"Please try again. Ctrl + C to exit."+Fore.RESET)
            print()

            code = oauth_exec(client)
            # This is a recursion. It is supposed to repeatedly ask for
            # credentials.
...
def oauth_exec(client):
    """
    Generates a link for the user to authorize the client on Baraag and
    prompts user for the authentication code.
    
    Takes 1 argument:
        
    client = Mastodon client generated by the initialization loop
            REQUIRED
    
    Returns the code input by the user.
...
def cold_init():
    ...
    try: 
        ...
        print()
        print("Requesting user authentication...")
        print()
        code = oauth_exec(client)
...
def write_checkpoint(key=None):
    ...
    If given no arguments, writes a blank checkpoint file.
   
    Returns nothing, writes checkpoint to disk.
    """
    if not key:
...
def process_following_user(client, settings, follow_dic, checkpoint=None, \
                           full=False):
    """
    Goes over a list of users, collects all posts with 
    ...
    completed.
    """
    # Initialize Checkpoint file IF Full mode (full == True)
    if full:
        if not checkpoint:
            write_checkpoint()
        ...
        # Single-user mode still requires a checkpoint that is not None
        checkpoint = []
...
def select_menu(logged_in):
    ...
    It takes 1 argument, a boolean generated by validate_login() representing
...
def validate_ini(settings):
    ...
    Takes 1 argument:
        ...
        settings = a list of settings generated by read_ini().
...
def download_list_init():
    """
    Simple function to spit out an empty download_list file if one does
    not already exist.
    This is mostly to save users the need to create a file themselves.
    
    Takes no arguments, creates a file on disk and returns nothing.
    """
...
def read_download_list():
    """  
    Returns a "sanitized" Python list of all accounts inside the download_list
    file.
    """
...
def parse_download_list(client, download_list):
    """
    Iterates over a list of accounts to download, and for each one returns
    a proper download list that can be passed on to download_following(),
    in the format: {account['acct']: {'account': account['acct'], 'id': account['id']}
                    
    Requires 2 arguments:
    
    client: Mastodon client generated by initialize(). REQUIRED
    
    download_list: list of accounts generated by read_download_list() REQUIRED
    
    Returns a dictionary with the proper format for download_following()
    """
    parsed_list = []
