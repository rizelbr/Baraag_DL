...
def oauth_exec(client):
    """
    Generates a link for the user to authorize the client on Baraag.
    
    Takes 1 argument:
        
    client = Mastodon client generated by the initialization loop
    
    Requires the user to input the code generated by the Baraag authorization.
...
def process_following_user(client, settings, follow_dic, checkpoint=None, \
    ...
    completed.
    """
    # Initialize Checkpoint file IF Full mode
    if full:
        if not checkpoint:
            write_checkpoint()
